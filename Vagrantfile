...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :master do |master|
    ...
    master.vm.provision "shell", name: "Apt Installs", inline: "apt-get update && apt-get upgrade -y && apt-get install -y apt-transport-https ca-certificates curl software-properties-common ntp ntpdate ntp-doc salt-api salt-cloud salt-master salt-minion salt-ssh salt-ssh avahi-daemon libnss-mdns"
