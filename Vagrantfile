...
if ENV['NUM_WORKERS']
  numWorkers = ENV['NUM_WORKERS'].to_i
else
  numWorkers = 4
end
Vagrant.configure("2") do |config|
  ...
  if RUBY_PLATFORM.include? 'linux' and !File.exist?("#{VAGRANT_ROOT}/provision-scripts/heketi/heketi.pub") 
    sh = Shell.new
    targetAdapter = sh.system("ssh-keygen -t rsa -f #{VAGRANT_ROOT}/provision-scripts/heketi/heketi -N '' -C heketi")
  end
  ...
  config.vm.define :master do |master|
    ...
    master.vm.provision "Set-Grains", type: :shell, path: "./provision-scripts/controller-grain.sh" 
    ...
    master.vm.provision "Adobe-NTP", type: :shell, path: "./provision-scripts/ntp-adobe.sh"

    master.vm.provision "Copy-master-confs", type: :file, source: "./provision-scripts/master-configs", destination: "~/"
    ...
    master.vm.provision "Get-Cert-Tools", type: :shell, path: "./provision-scripts/getCertTools.sh" 
    hostip=''
    ...
    template = ERB.new File.read("#{VAGRANT_ROOT}/provision-scripts/heketi/heketi.json.erb")
    heketitopology = template.result(binding)

    master.vm.provision "Heketi Files", type: :file, source: "./provision-scripts/heketi/", destination: "~/heketi"
    ...
    master.vm.provision "Heketi Install", type: :shell, path: "./provision-scripts/heketi-install.sh", args: ["/home/ubuntu/heketi/heketi.pub","/home/ubuntu/heketi/heketi", "/home/ubuntu/heketi/heketi.service", "/home/ubuntu/heketi/heketi.json"]
    ...
    master.vm.provision "Salt-Num-Workers", type: :shell, inline: "echo -e 'kubernetes:\\n  num-workers: #{numWorkers}' > /etc/salt/base-file-root/pillar_root/num-workers.sls" 

    master.vm.provision "Copy-Cert-Json", type: :file, source: "./provision-scripts/cert-generation/", destination: "~/cert-generation"
    master.vm.provision "Gen-Certs", type: :shell, path: "./provision-scripts/generateCerts.sh", args: ["10.0.0.1,master.local,#{networkPrefix}.100,#{hostip}", "/etc/salt/base-file-root/file_root/certs"], env: {"NUM_WORKERS" => "#{numWorkers}"}
  end
  ...
  (1..numWorkers).each do |i|
    config.vm.define "node-#{i}" do |node|
      ...
      node.vm.provision "Set-Grains", type: :shell, path: "./provision-scripts/worker-grain.sh" 
      ...
      #node.vm.provision "Add-Disks", type: :shell, path: "./provision-scripts/vagrant-prepdrive.sh"

      node.vm.provision "Heketi SSH Public Key", type: :file, source: "./provision-scripts/heketi/heketi.pub", destination: "~/heketi.pub"
      ...
      node.vm.provision "Config-NTP", type: :shell, path: "./provision-scripts/ntp-adobe.sh"
      ...
      node.vm.provision "Gluster-Join", type: :file, source: "./provision-scripts/gluster-install.sh", destination: "~/gluster-install.sh"
      node.vm.provision "Setup GlusterFS", type: :shell, inline: "chmod +x /home/ubuntu/gluster-install.sh; nohup /home/ubuntu/gluster-install.sh > /var/log/gluster-vagrant-up.log &", env: {"NUM_WORKERS" => "#{numWorkers}"}
    end
  end
end
