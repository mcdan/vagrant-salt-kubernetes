...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :master do |master|
    ...
    master.vm.provision "Apt-Install", type: :shell, inline: "apt-get update && apt-get upgrade -y && apt-get install -y rng-tools apt-transport-https ca-certificates curl software-properties-common ntp ntpdate ntp-doc salt-api salt-cloud salt-master salt-minion salt-ssh salt-ssh avahi-daemon libnss-mdns git python-pygit2"
    ...
    master.vm.provision "Gen-Certs", type: :shell, path: "./generateCerts.sh", args: ["master.local,10.0.0.10,#{hostip}", "/etc/salt/base-file-root/file_root/certs"] 
  end
  ...
  (1..numSlaves).each do |i|
    config.vm.define "node-#{i}" do |node|
      ...
      node.vm.provision "Apt-Install", type: :shell, inline: "apt-get update && apt-get upgrade -y && apt-get install -y rng-tools apt-transport-https ca-certificates curl software-properties-common ntp ntpdate ntp-doc salt-minion avahi-daemon libnss-mdns"

      node.vm.provider "virtualbox" do |vb|
        disk = "./disks/storage-cluster-n#{i}.vdi"
        unless File.exist?(disk)
          vb.customize ['createmedium', 'disk', '--filename', disk, '--variant', 'Fixed', '--size', 20 * 1024]
          vb.customize ['storageattach', :id,  '--storagectl', 'SCSI', '--port', 4, '--device', 0, '--type', 'hdd', '--medium', disk]
        end
      end
